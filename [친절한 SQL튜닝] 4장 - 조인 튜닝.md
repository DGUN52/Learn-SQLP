...
- 필터 오퍼레이션
  ...
  - NL조인과 처리루틴이 같으나 다음과 같은 차이점 존재
    ...
    2) 필터는 캐싱기능을 갖는다 : 서브쿼리의 입력값에 따른 반환 값(true/false)을 캐싱하여 반복된 값에 대한 작동여부를 판단
...
4.4.4 스칼라 서브쿼리 조인
- 스칼라 서브쿼리의 특징
  - GET_DNAME이라는 dept테이블의 dname을 select문의 deptno조건으로 구하는 함수가 있다면
  - 함수안에 있는 select 쿼리를 메인쿼리 건수만큼 재귀적으로 반복 실행한다.
  - 반면 GET_DNAME이라는 함수가 들어갈 자리에 (select dname from dept d where d.deptno = e.deptno)라는 스칼라 서브쿼리로 대체할 때는 재귀적으로 실행하지 않는다.
    - 스칼라 서브쿼리와 메인쿼리를 한 몸체처럼 실행한다.
    - Outer 조인문처럼 NL조인 방식으로 실행된다.
      - 조인에 실패하는 emp 레코드는 dname값이 null을 출력한다는 점도 outer 조인과 같다.
      - 차이점은 스칼라 서브쿼리는 처리과정에서 캐싱 작용이 발생한다.

- 스칼라 서브쿼리의 캐싱 효과
  - 스칼라 서브쿼리로 조인하면 입력값과 출력값이 Query Excution Cache에 캐싱된다.
  - 조인 시 캐시에 없을 때만 조인을 수행하고 결과는 캐시에 저장한다.
  - 스칼라 서브쿼리의 입력값은 서브쿼리에서 참조하는 메인쿼리의 컬럼값이다. (위 서브쿼리의 특징에서 기술한 select문의 e.deptno)
  - 스칼라 서브쿼리의 캐싱은 필터 서브쿼리의 캐싱과 같은 기능
  - 캐싱은 PGA메모리에서 진행된다. 쿼리가 끝나면 공간이 반환된다.
  - 따라서 사용자 함수 대신 스칼라 서브쿼리를 사용하는 것이 성능에 도움된다.

- 스칼라 서브쿼리의 캐싱 부작용
  - 캐싱공간은 매우 작은 공간이기에 해시 충돌 가능성이 작을 때 효과적이다.
  - 반대로 캐시의 입력값이 다양하다면 캐시를 매번 확인하는 비용만 추가적으로 들 뿐이다. (cpu, 메모리, 수행시간 증가)
  - 또한 메인 쿼리의 집합이 매우 작다면 재사용성이 낮아서 성능에 도움이 되지 못한다.(캐시 갱신으로 불필요한 리소스만 낭비한다.)
 
- 두 개 이상의 값 반환
  - 스칼라 서브쿼리가 포함된 쿼리의 실행계획
    - 스칼라 서브쿼리 부분이 위에 있고 메인쿼리 부분이 아래에 있다.
    - 표현방식은 NL조인과 다르지만 프로세싱 과정은 같다. 부분범위 처리도 가능하다.
    - NL조인과 차이점은 캐싱 효과가 있다는 것
  - 스칼라 서브쿼리를 이용하면서 값을 여러개 반환하고 싶다면
    - 하나의 서브쿼리 문에서 lpad같은 문자열 연산으로 모두 결합한 후 substr같은 함수로 분리하는 방식이 있다.
    - Object 타입을 이용하는 방법도 있지만 Type 선언에 대한 불편함이 있다.
    - 이런저런 고민 없이 인라인 뷰를 사용해도 된다.
      - 다만 뷰를 사용할 시 머지에 대한 고민이나 group by로 인한 부분범위 처리에 대한 고민이 존재한다.

- 스칼라 서브쿼리 unnesting
  - 스칼라 서브쿼리의 장점인 캐싱 효과가 크지 않으면 NL조인과 마찬가지로 랜덤IO부담이 있다.
  - 혹은 병렬 쿼리에선 스칼라 서브쿼리를 되도록 사용하지 말아야 한다. 해시 조인으로 처리해야 효과적이기 때문.
  - 따라서 서브쿼리를 일반 조인문으로 변환하고자 할 때 스칼라 서브쿼리 unnesting 기능이 있다. (12c 이후) 
    - 스칼라 서브쿼리를 unnesting 할 시 스칼라 서브쿼리임에도 NL조인이 아닌 해시조인으로 실행할 수 있다.
    - unnesting으로 인해 문제가 생겼을 시 no_unnest 힌트로 언네스팅을 막을 수 있다.
